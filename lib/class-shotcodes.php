...
final class IVD_Shortcodes{
...

    {
        ...
        if (wp_get_current_user()->exists()) {
            ...
            return '<div class="col-sm-12"><label>[:en]Course currency exchange[:ru]Текущий курс обмена[:]: </label><input style="min-width:200px;" type="text" readonly value="' . sanitize_text_field($course ). '"><label>[:en]for 1 token '.self::$name_of_unit.'[:ru]за 1 токен МСЕ[:]</label></div>';
